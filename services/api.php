<?php
	...
	class API extends REST {
		...
		const DB_USER = "karwartw_admin";
		const DB_PASSWORD = "bhandari@1988";
		//const DB = "angularcode_customer";
		const DB = "karwartw_bhandarisangh";
		...
		*/
		...
		private function login(){
			...
			if(!empty($email) and !empty($password)){
				if(filter_var($email, FILTER_VALIDATE_EMAIL)){
					...
					$this->response('Invalid username and password', 204);	// If no records "No Content" status
				}
			}
		...
		private function sessionLogin()
		{
			...
			$username = (string)$customer['id'];
			$password = (string)$customer['pwd'];

			/*$username = 'm_rajesh75';
			$password = '123456';*/
			//$username = $this->_request['id'];		
			//$password = $this->_request['pwd'];

			//$query="SELECT distinct FROM 'matromonial' WHERE prof_userId = '$username' AND prof_password = '".md5($password)."' LIMIT 1";
			$query = "SELECT * FROM `matromonial` WHERE `prof_userId`= '$username' AND `prof_password`= '$password'";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
			if($r->num_rows > 0){
				$result = array();
		...
		private function getProfiles(){
			...
			$gender = json_decode(file_get_contents("php://input"),true);
			var_dump("post data:: " + $gender);
			$query="SELECT * FROM `matromonial` WHERE prof_gender='$gender'";
